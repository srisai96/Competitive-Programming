...
def max_duffel_bag_value(cake_tuples, weight_capacity):
    ...
    arr = []
    for x in range(0,weight_capacity+1):
        arr.append(0)

    if len(cake_tuples)==1 and cake_tuples[0][0]==0:
        return float('inf')
    for i in range(0,len(arr)):
        for j in range(0,len(cake_tuples)):
            if cake_tuples[j][0] <= i:
                arr[i] = max(arr[i], cake_tuples[j][1] + arr[(i - cake_tuples[j][0])])
    return arr[-1]
